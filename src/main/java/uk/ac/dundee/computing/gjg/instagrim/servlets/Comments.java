...
public class Comments extends HttpServlet {
    ...
    protected void DisplayPicComments(java.util.UUID picid, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        java.util.List<String> lsComments = tm.getComments(picid); //get all comments for picture of given id

        alComments.addAll(lsComments);

        RequestDispatcher rd = request.getRequestDispatcher("/comments.jsp"); 
        request.setAttribute("PicComments", alComments); //send list comment's jsp
        rd.forward(request, response); //transfer control 
